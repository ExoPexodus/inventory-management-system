# Unified Product Creation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the basic `/entries` product creation page with a unified 4-tab form (Details / Images / Variants / Prices) where merchants can fill in everything before a single API call fires.

**Architecture:** Single `"use client"` page at `apps/admin-web/src/app/(main)/entries/page.tsx` (complete rewrite). All form data staged in React `useState`. On "Save product": create product → upload images sequentially → POST variants → POST prices. Partial failures after product creation surface as non-blocking warnings and don't prevent navigation. Uses existing `Tabs`, `Toggle`, `TextInput`, `SelectInput`, `Badge`, `PrimaryButton` from `@/components/ui/primitives`.

**Tech Stack:** Next.js 15 / React 19 / TypeScript / Tailwind. Hooks: `useCurrency()` from `@/lib/currency-context`, `useRouter` from `next/navigation`. No backend changes.
...
### DropZone limitation
`DropZone` from primitives only accepts a single file (`<input type="file">`). The Images tab needs multi-file support — build a custom drop zone inline rather than extending the primitive.

### Tabs primitive
`Tabs({ tabs: Array<{ id: string; label: string }>, active: string, onChange: (id: string) => void })`

### Toggle primitive
`Toggle({ checked: boolean, onChange: (value: boolean) => void })`

### Image upload flow (from storage quota work)
```
POST /api/ims/v1/admin/media/presign-upload
  Body: { folder, filename, content_type, file_size_bytes }
  → { upload_url, public_url, storage_warning? }

PUT upload_url  (file bytes, Content-Type: file.type)

POST /api/ims/v1/admin/catalog/products/{id}/images
  Body: { url: public_url, sort_order: number, file_size_bytes: file.size }
```

### Variant create
`POST /api/ims/v1/admin/products/{id}/variants`
Body: `{ sku, name, unit_price_cents, options: Record<string,string> }`

### Price create
`POST /api/ims/v1/admin/products/{id}/prices`
Body: `{ currency_code, amount_cents, channel_id: null }`

### Product create (existing, from current /entries)
`POST /api/ims/v1/admin/products`
Body: `{ sku, name, unit_price_cents, category, barcode, cost_price_cents, mrp_cents, hsn_code, negative_inventory_allowed, variant_label?, product_group_id?, status }`
...
## Task 1: Write the unified product creation page

**Files:**
- Modify: `apps/admin-web/src/app/(main)/entries/page.tsx`

- [ ] **Step 1: Replace the file with this complete implementation**

Write `apps/admin-web/src/app/(main)/entries/page.tsx`:
...
function ImagesSection({
  ...
  function handleFiles(files: FileList | null) {
    ...
    if (stagedImages.length + valid.length > 10) {
      onAdd(valid.slice(0, 10 - stagedImages.length));
    } else {
      onAdd(valid);
    }
  }
...
export default function EntriesPage() {
  ...
  // Revoke preview URLs on unmount to prevent memory leaks
  useEffect(() => {
    return () => {
...
Expected: `✓ Compiled successfully`. Common TS errors and fixes:
- `'img' element without alt` → the `alt` attribute is present, but if Next.js ESLint complains about `<img>`, either ignore with `{/* eslint-disable-next-line @next/next/no-img-element */}` (already in code) or replace with `<Image>` from `next/image` — the simpler suppression is fine here.
- `stagedImages[i]` may be undefined → add `!` non-null assertion (already in code).

- [ ] **Step 3: Smoke test in browser**

Navigate to `http://localhost:3100/entries` (or via "Create Product" in the New Entry menu):

1. **Details tab** — form fields match current `/entries`. Price guard warning appears when price < cost. Required fields (SKU, Name, Price) block save if empty.
2. **Images tab** — drag-drop or browse. Preview cards appear. × removes a card. Badge count in tab label increments.
3. **Variants tab** — fill form, click "Add variant" → row appears in table. Delete icon removes a row.
4. **Prices tab** — add a USD price → appears in list. Delete removes it.
5. **Footer** — shows counts summary. Click "Save product →" with required fields filled → spinner shows, progress message updates, redirects to `/products`.

- [ ] **Step 4: Commit**

```bash
git add 'apps/admin-web/src/app/(main)/entries/page.tsx'
git commit -m "feat(admin-web): unified product creation with Details/Images/Variants/Prices tabs"
```

---

## Task 2: Full rebuild + push
...
- [ ] **Step 2: Backend tests (no backend changes, verify no regression)**

```bash
CONTAINER=$(docker compose ps -q api)
docker compose exec api pip install pytest -q 2>&1 | tail -1
...
|---|---|
| Replaces `/entries/page.tsx` in-place | Task 1 |
| 4 tabs: Details / Images / Variants / Prices (freely navigable) | Task 1 (`Tabs` component, `activeTab` state) |
| Details fields identical to current form | Task 1 (`DetailsSection`) |
| Status field (active/draft) | Task 1 (`DetailsSection`) |
| Images: drag-and-drop + file picker, multi-file, preview cards, × to remove | Task 1 (`ImagesSection`) |
| 10 MB per file / 10 image limit enforced client-side | Task 1 (`handleFiles`) |
| Variants: inline add form → staged table | Task 1 (`VariantsSection`) |
| Prices: inline add form → staged list | Task 1 (`PricesSection`) |
| Save sequence: product → images → variants → prices | Task 1 (`handleSave`) |
| Product failure: inline error, stay on page | Task 1 |
| Image failure: non-blocking warning, continue | Task 1 |
| Variant failure: non-blocking warning, continue | Task 1 |
| Price failure: non-blocking warning, continue | Task 1 |
| Progress message during save | Task 1 (`saveProgress`) |
| `router.push('/products')` on completion | Task 1 |
| `file_size_bytes` in presign + image save (from storage quota work) | Task 1 |
| Tab labels show count badge when items are staged | Task 1 |
| `URL.revokeObjectURL` cleanup on unmount | Task 1 |

**Placeholder scan:** None.

**Type consistency:**
- `StagedImage { file, previewUrl }` → used in `ImagesSection` and `handleSave` consistently ✅
- `StagedVariant { sku, name, price, optionPairs }` → used in `VariantsSection` and `handleSave` consistently ✅
- `StagedPrice { currencyCode, amount }` → used in `PricesSection` and `handleSave` consistently ✅
- `amount_cents: Math.round(parseFloat(p.amount) * 100)` — consistent with how the existing PricesModal calculates cents ✅
- `unit_price_cents: Math.round(parseFloat(v.price) * 100)` — consistent with existing VariantsModal ✅
